...
/**
 * @title LiquidAccountConduitSimple
 * @notice Simplified conduit for claiming only option tokens and exercising them
 * @dev Simplified flow:
 *      1) Claim option tokens for `user` from each gauge to this contract
 ...
 */
contract LiquidAccountConduitSimple is AccessControl, IERC721Receiver {
    ...
    /**
     * @notice Claims option tokens from gauges, exercises them, and optionally merges the new veNFT
    ...
    function claimExerciseAndMerge(
        ...
        // We claim from each gauge individually and emit per-gauge events to track individual gauge claims for the user
